...
function checkOverview(Email $email, $expectIsset)
{
    if ($expectIsset) {
...
function checkBody(Email $email, $expectIsset)
{
    if ($expectIsset) {
...
function checkHeader(Email $email, $expectIsset)
{
    if ($expectIsset) {
...
class EmailPartFetchTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testFetchNoParts()
    {
        $this->downloader->fetch($this->criteria, function (Email $email) {
            checkOverview($email, false);
            checkBody($email, false);
            checkHeader($email, false);
        }, 0);
    }
